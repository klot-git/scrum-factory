...
namespace ScrumFactory.Services.Logic {
    ...
    public class MailerService : IMailerService {
        ...
        public bool SendEmail(Project project, string subject, string body, bool PONotification = false) {
            List<string> teamEmails = new List<string>();

            ProjectMembership[] members = null;
            if(!PONotification)
            {
                members = project.Memberships.Where(ms => ms.IsActive && ms.Member != null && !String.IsNullOrEmpty(ms.Member.EmailAccount) && ms.Member.IsContactMember == false).ToArray();
            } else
            {
                members = project.Memberships.Where(ms => ms.IsActive && ms.Member != null && !String.IsNullOrEmpty(ms.Member.EmailAccount) && ms.Role.PermissionSet == (short)ScrumFactory.PermissionSets.PRODUCT_OWNER).ToArray();
            }

            foreach (ProjectMembership ms in members) {
                if (!teamEmails.Contains(ms.Member.EmailAccount))
