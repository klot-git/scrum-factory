...
namespace ScrumFactory.Services.Logic {
    ...
    public class TeamService : ITeamService_ServerSide {
        ...
        [WebGet(UriTemplate = "Members/?filter={filter}&availability={availability}&clientName={clientName}&activeOnly={activeOnly}&workingWith={workingWith}&top={top}&includeProjects={includeProjects}&includeProposals={includeProposals}&includeSupport={includeSupport}", ResponseFormat = WebMessageFormat.Json)]
        public ICollection<MemberProfile> GetMembers(string filter, int availability, string clientName, bool activeOnly, string workingWith, int top, bool includeProjects = false, bool includeProposals = false, bool includeSupport = false) {
            ...
            ICollection<MemberProfile> members = teamRepository.GetAllMembers(filter, availability, companies.ToArray(), activeOnly, workingWith, top, includeProjects, includeProposals, includeSupport);
        ...
        public ICollection<MemberProfile> GetContacts(string clientName) {
            authorizationService.VerifyRequestAuthorizationToken();            
            return teamRepository.GetAllMembers(null, 0, new string[] { clientName }, true, null, 0, false, false, false);
        }
