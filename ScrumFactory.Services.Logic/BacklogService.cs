...
namespace ScrumFactory.Services.Logic {
    ...
    public partial class BacklogService : IBacklogService, IBacklogService_ServerSide {
        ...
        public int AddBacklogItem(BacklogItem item) {
            return AddBacklogItem(item, false);
        }

        [WebInvoke(Method = "POST", UriTemplate = "BacklogItems/?notifyPO={notifyPO}", RequestFormat = WebMessageFormat.Json)]
        public int AddBacklogItem(BacklogItem item, bool notifyPO) {
            ...
            {                
                try
                {
                    SendTicketEmail(item, project, false);

                    if(notifyPO)
                    {
                        SendTicketEmail(item, project, true);
                    }
