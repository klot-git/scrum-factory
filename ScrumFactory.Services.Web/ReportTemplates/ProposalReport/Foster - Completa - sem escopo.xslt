...
<xsl:stylesheet
  ...
  <xsl:template match="/ReportData">
    <FlowDocument
      ...
      <Paragraph>
Os requisitos funcionais que compõem o escopo do projeto são apresentados abaixo.<LineBreak/>
As horas necessárias para a implementação de cada um deles é apresentada por recurso (em cinza) ao lado de sua descrição.<LineBreak/>
O custo de cada item é exibido ao final de cada linha.
      ...
      <xsl:if test="count(//ArrayOfProjectConstraint/ProjectConstraint) &gt; 0">
        <Paragraph Style="{{StaticResource TitleParagraph}}" KeepWithNext="True">
          8. <xsl:value-of select="$_CONSTRAINTS"/>
        </Paragraph>
      ...
      <xsl:if test="count(//Proposal/Clauses/ProposalClause) &gt; 0">
          ...
          <xsl:call-template name="proposalClauses">
            <xsl:with-param name="index" select="$clauseIdx"/>
	  </xsl:call-template>
        </xsl:if>
